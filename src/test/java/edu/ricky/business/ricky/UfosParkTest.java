...
public class UfosParkTest {
    ...
    public void dispatchTest() {
        ...
        List<String> cards = ufos.cardNumbers().stream().filter(n -> n == card.getNumber())
                .collect(Collectors.toList());

        assertEquals(1, cards.size(), 0);
        assertEquals(2500, card.getCredit(), 0);
    }
    ...
    public void dispatchNoCreditTest() {
        ...
        ufos.dispatch(card);
        assertFalse(ufos.containsCard(card.getNumber()));
        assertEquals(0, card.getCredit(), 0);
    }
    ...
    public void dispatchNoUfoAvaliableTest() {
        ...
        List<String> cards = ufos.cardNumbers().stream().filter(n -> n == morty.getNumber())
                .collect(Collectors.toList());

        assertEquals(0, cards.size(), 0);
        assertEquals(3000, morty.getCredit(), 0);
    }
    ...
    public void getUfoOfTest() {
        CreditCard card = new CreditCard("Abradolf Lincler", "4916119711304546");
        ufos.dispatch(card);
        assertTrue(ufos.toString().contains(ufos.getUfoOf(card.getNumber())));
        assertEquals(2500, card.getCredit(), 0);
    }
}